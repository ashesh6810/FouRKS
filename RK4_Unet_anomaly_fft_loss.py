...
#        output,_,_,_,_,_,_ = net(input_batch.cuda())
        output = RK4step(net,input_batch)
        loss = my_loss(output, label_batch.cuda(),wavenum_init,lambda_reg)
        loss.backward()
        optimizer.step()
        output_val = RK4step (net,psi_test_input_Tr_torch[0:num_samples].reshape([num_samples,2,dim_lon,96]))
        val_loss = my_loss(output_val, psi_test_label_Tr_torch[0:num_samples].reshape([num_samples,2,dim_lon,96]).cuda(),wavenum_init,lambda_reg)
        # print statistics
...
torch.save(net.state_dict(), './BNN_RK4UNET_anomaly_FFT_loss_lead'+str(lead)+'.pt')
...
savenc(autoreg_pred, lon, new_lat, path_outputs+'predicted_RK4_FFT_anomaly_waveinit_'+str(wavenum_init)+'_lead'+str(lead)+'.nc')
