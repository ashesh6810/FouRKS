...
#        output,_,_,_,_,_,_ = net(input_batch.cuda())
        output,_,_,_,_,_,_ = net (input_batch_large.cuda())
        loss = my_loss(output, label_batch_small_scale.cuda(),wavenum_init,lambda_reg)
        loss.backward()
        optimizer.step()
        output_val,_,_,_,_,_,_ = net(psi_test_input_Tr_torch_large[0].reshape([1,2,dim_lon,96]).cuda())
        val_loss = my_loss(output_val,label_batch_small_scale[0].reshape([1,2,dim_lon,96]).cuda(),wavenum_init,lambda_reg)
        # print statistics
...
torch.save(net.state_dict(), './BNN_UNET_anomaly_large_scale_to_small_scale_low_pass_'+str(lim)+'_FFT_loss_lead'+str(lead)+'.pt')
